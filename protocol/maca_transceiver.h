...
#ifndef __maca_transceiver_h
#define __maca_transceiver_h

#include <machine.h>

__BEGIN_SYS

class MACA_Transceiver
{
private:
    ...
    enum Register_Bits {
        ...
        // MACA_IRQ
        IRQ_STRT = 15, // ActionStarted
        IRQ_SYNC = 14, // SyncFound
        IRQ_CM   = 13, // Timeout
        IRQ_CRC  = 12, // CrcFail
        IRQ_FLT  = 11, // FilterFail
        IRQ_SFT  = 10, // SoftTimeout
        IRQ_LVL  = 9,  // RxLevel 
        IRQ_RST  = 4,  // Reserved 
        IRQ_WU   = 3,  // Reserved
        IRQ_DI   = 2,  // DataIndication
        IRQ_POLL = 1,  // PollRx
        IRQ_ACPL = 0,  // ActionComplete
    ...
    enum {
		...
		SYNC_BCN // Not part of MACA! Used by maca_transceiver
    };
...
public:
    ...
    static void post_tx_beacon();

    static void post_receive(unsigned int timeout);
    static void post_tx();
    ...
    static void set_event_handler(event_handler ev_handler);
...
public:
	...
	static int act_idx;
	static unsigned int current_slot_end;
    static const unsigned int CLOCK_OFFSET = 50;
    static const unsigned int TSTXOFFSET = 50 + CLOCK_OFFSET;
    static const unsigned int TIMEOUT_OFFSET = 100;
    static const unsigned int MAX_FRAME_SIZE = 12 + 2 + 1; // Reserved space for FCS and length
	static const unsigned int n_actions = 12;
	static const unsigned int n_slots = 3;

	static const unsigned int BCN_WAIT_TIMEOUT = 1000000000; // us 
	//static const unsigned int BCN_WAIT_TIMEOUT = 50000; 
	static void force_sync(unsigned int timeout_us = BCN_WAIT_TIMEOUT); 
	static void run();
	static void set_tx_buffer(unsigned char * buffer, unsigned int sz); 

	static const int actions[n_actions];//={SYNC_BCN, SEQ_TX, SEQ_WAIT, SYNC_BCN, SEQ_RX, SEQ_WAIT, SYNC_BCN, SEQ_RX, DEQ_WAIT, WYNC_BCN, SEQ_RX, SEQ_WAIT};
	static const unsigned int slot_size[n_slots];// = {1000, 1000, 500}; //4ms, 4ms, 2ms
	static const unsigned int send_buffer_size = 5 + 2; //payload + checksum
	volatile static unsigned char rx_buffer[MAX_FRAME_SIZE];
