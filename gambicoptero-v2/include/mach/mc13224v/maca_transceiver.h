...
private:
    ...
    enum {
		...
		BCN_RX, // Not part of MACA! Used by maca_transceiver
		BCN_TX, // Not part of MACA! Used by maca_transceiver
    };
...
public:
	...
	static const unsigned int BCN_WAIT_TIMEOUT = 250000; // 1 second
	//static const unsigned int BCN_WAIT_TIMEOUT = 2500 * 10; // 10 superframes
	static void run();
	static void set_tx_buffer(volatile unsigned char * buffer, unsigned int sz); 

	static const int actions[n_actions];
	static const unsigned int slot_size[n_slots];
	static const unsigned int send_buffer_size = 5 + 2; //payload + checksum
	volatile static unsigned char rx_buffer[MAX_FRAME_SIZE];
	volatile static unsigned char dummy_buffer[MAX_FRAME_SIZE];
